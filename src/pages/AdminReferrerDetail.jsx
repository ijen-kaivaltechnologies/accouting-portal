...
const AdminReferrerDetail = () => {
	...
	const handleAction = async (status) => {
		if (!window.confirm(`Are you sure you want to ${status} this referrer?`))
			return;
		setProcessing(true);
		try {
			await api.updateAdminReferrerStatus(id, {
				status,
	...
	const handleSaveNotes = async () => {
		setNotesSaving(true);
		try {
			await api.updateAdminReferrerNote(id, { admin_note: adminNote });
			alert("Note saved!");
			fetchReferrer();
		} catch (err) {
	...
	const handleDelete = async () => {
		if (
			!window.confirm(
				`Are you sure you want to delete referrer "${referrer.full_name}"? This will also delete all their related data (requests, documents, etc.) and cannot be undone.`,
			)
		) {
			return;
		}
		try {
			await api.deleteAdminReferrer(id);
			alert(`Referrer "${referrer.full_name}" deleted successfully!`);
			navigate("/admin/referrers");
		} catch (err) {
			console.error(err);
			alert(err.response?.data?.error || "Failed to delete referrer");
		}
	};
	...
	const initials = referrer.full_name
		?.split(" ")
		.map((n) => n[0])
		.join("")
		.toUpperCase()
		...
		<div style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
			...
			<div
					...
					<div
						...
						<div style={{ padding: "8px 20px 12px" }}>
							...
							<InfoItem
								...
								value={
									referrer.dob
										? new Date(referrer.dob).toLocaleDateString("en-IN")
										: null
								}
							/>
							<InfoItem
								icon={Hash}
								label="PAN Number"
								value={referrer.pan_no}
							/>
							<InfoItem
								icon={CreditCard}
								label="Aadhar Number"
								value={referrer.aadhar_no}
							/>
							<InfoItem
								icon={MapPin}
								label="Address"
							...
							<InfoItem
								icon={Calendar}
								label="Registered On"
								value={new Date(referrer.created_at).toLocaleString("en-IN")}
							/>
						</div>
					</div>
							...
							{[
								{
									type: "aadhar",
									label: "Aadhar Card",
									pathKey: "aadhar_file_path",
								},
								{ type: "pan", label: "PAN Card", pathKey: "pan_file_path" },
								{
									type: "cheque",
									label: "Bank Cancelled Cheque",
									pathKey: "bank_cancel_check_path",
								},
							].map((doc) => {
								const hasDoc =
									referrer[doc.pathKey] && referrer[doc.pathKey] !== "PENDING";
								return (
									<div
										key={doc.type}
										style={{
				...
				<div
					...
					<div
						style={{ padding: "15px 20px", borderBottom: "1px solid #f1f5f9" }}>
						<h2
					...
					<div style={{ padding: "18px 20px" }}>
						{!isPending && (
							<div
								style={{
									background:
										referrer.status === "approved" ? "#f0fdf4" : "#fef2f2",
									border: `1px solid ${referrer.status === "approved" ? "#bbf7d0" : "#fecaca"}`,
									borderRadius: "10px",
									padding: "14px",
									marginBottom: "14px",
								}}>
								<p
									style={{
										...
										fontWeight: "700",
										color:
											referrer.status === "approved" ? "#16a34a" : "#dc2626",
										textTransform: "uppercase",
										letterSpacing: "0.05em",
										marginBottom: "8px",
									}}>
								...
								{referrer.reviewed_at && (
									<p
										...
										Reviewed on{" "}
										{new Date(referrer.reviewed_at).toLocaleString("en-IN")}
									</p>
								)}
							</div>
						)}
						<div
							...
							<button
								...
								<Trash2 size={15} /> Delete Referrer
							</button>
						</div>
					</div>
				</div>
